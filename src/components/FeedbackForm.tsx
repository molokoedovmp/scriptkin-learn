...
export function FeedbackForm({
  defaultTopic = "Предложение",
  context,
  compact = false,
  onDone,
}: {
  defaultTopic?: "Предложение" | "Ошибка на сайте" | "Вопрос" | "Другое";
  context?: string;
  compact?: boolean;
  onDone?: () => void;
} = {}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      message: [context, form.get("message")].filter(Boolean).join("\n\n"),
  ...
  if (sent) {
    return (
      <div className={`rounded-[24px] border-2 border-[#cceeb4] bg-storybook-green p-8 text-center ${compact ? "" : "sm:p-12"}`}>
        ...
        <button onClick={onDone ?? (() => setSent(false))} className="mt-6 rounded-xl border-2 border-[#78bd42] bg-paper-white px-5 py-3 text-caption font-extrabold uppercase text-[#3f9900]">
          {onDone ? "Закрыть" : "Отправить ещё"}
        </button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className={`rounded-[24px] border-2 border-[#e6e7eb] bg-paper-white p-5 shadow-[0_10px_30px_rgba(0,4,55,0.06)] ${compact ? "" : "sm:p-8"}`}>
      ...
      <label className="mt-5 block">
        ...
        <select name="topic" required defaultValue={defaultTopic} className="w-full rounded-xl border-2 border-[#dedede] bg-paper-white px-4 py-3 text-body font-bold text-charcoal outline-none focus:border-spark-blue">
      ...
      <label className="mt-5 block">
        <span className="mb-2 flex justify-between gap-3 text-caption font-extrabold uppercase text-charcoal">
          Сообщение <span className="normal-case text-faded-gray">до {Math.max(10, 2998 - (context?.length ?? 0))} символов</span>
        </span>
        <textarea name="message" required minLength={10} maxLength={Math.max(10, 2998 - (context?.length ?? 0))} rows={compact ? 5 : 8} placeholder={compact ? "Опиши, что произошло, что ожидалось и как повторить ошибку…" : "Расскажи, что понравилось, что не работает или чего не хватает…"} className="w-full resize-y rounded-xl border-2 border-[#dedede] bg-paper-white p-4 text-body font-medium text-charcoal outline-none focus:border-spark-blue" />
      </label>
      ...
      <button type="submit" disabled={loading} className={`mt-6 w-full rounded-xl bg-eager-green px-6 py-4 text-nav-label font-extrabold uppercase text-paper-white shadow-[0_5px_0_#3f9900] disabled:opacity-50 ${compact ? "" : "sm:w-auto"}`}>
