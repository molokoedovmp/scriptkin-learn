...
export const metadata: Metadata = {
  title: "Сообщество — личный кабинет",
};
...
export default async function AccountCommunityPage({
  ...
  if (!user) redirect("/login?returnTo=/account/community");

  const activeTag = POST_TAGS.some((item) => item.id === tag) ? tag! : null;
  const community = await getCommunityPosts(user.id, activeTag);
  const basePath = "/account/community";

  return (
    <AccountShell>
      <AccountSectionHeader
        title="Сообщество"
        description="Общая лента решений, вопросов и учебного прогресса пользователей."
      />

      <div className="grid min-w-0 items-start gap-4 lg:grid-cols-[230px_minmax(0,760px)] lg:gap-6">
        <aside className="min-w-0 overflow-hidden rounded-[16px] border-2 border-[#e6e7eb] bg-paper-white p-2.5 lg:sticky lg:top-6 lg:p-3">
        ...
        <section className="min-w-0 rounded-[20px] border-2 border-[#e6e7eb] bg-paper-white p-4 sm:p-6">
          {community.posts.length === 0 ? (
            <div className="rounded-xl bg-[#f7f8fa] p-10 text-center">
              ...
              <p className="mt-3 font-extrabold text-charcoal">
                По этому тегу публикаций пока нет
              </p>
              <Link
                href={basePath}
                className="mt-3 inline-flex font-extrabold text-spark-blue hover:underline"
              >
                Показать всю ленту
              </Link>
            </div>
              ...
              {community.posts.map((post) => (
                <article key={post.id} className="py-6 first:pt-0 last:pb-0">
                  <div className="flex gap-3">
                    <Link
                      href={`/account/users/${post.authorId}`}
                      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#eee9ff] font-black text-[#7449bd]"
                    >
                    ...
                    <div className="min-w-0 max-w-[680px] grow">
                      <Link
                        href={`/account/users/${post.authorId}`}
                        className="font-extrabold text-charcoal hover:text-spark-blue"
                      >
                        {post.authorName}
                      </Link>
                      <p className="text-caption font-bold text-faded-gray">
