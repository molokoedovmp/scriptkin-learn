...
export default async function AccountActivityPage() {
  ...
  return <><Header /><main className="min-w-0 flex-1 bg-[#f7f8fa]"><AccountShell><AccountSectionHeader title="Активность" description="Календарь занятий в стиле GitHub: шаги историй и решённые упражнения за каждый день." /><div className="grid min-w-0 items-start gap-5 xl:grid-cols-[minmax(0,3fr)_minmax(230px,1fr)]"><div className="min-w-0"><AccountActivityView activity={social.activity} /></div><AccountFriendsSummary friends={social.friends} requests={social.requests} /></div></AccountShell></main><Footer /></>;
}
